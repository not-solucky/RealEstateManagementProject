...
func (h *Handler) handleDashSubmitDocument(w http.ResponseWriter, r *http.Request) {
	...
	var payload types.UserDocumentPayload
	...
	documentID, err := h.store.GetDocumentID(payload.UserID)
	if err != nil {
...
func (h *Handler) handleVerifyUser(w http.ResponseWriter, r *http.Request) {
	contextValues := r.Context().Value(auth.UserKey).(types.UserContext)
	...
	var payload types.UserVerifyPayload
	if err := utils.ParseJSON(r, &payload); err != nil {
		fmt.Println("error")
		utils.WriteError(w, http.StatusBadRequest, err)
		return
	...
	err := h.store.VerifyUser(payload)
	if err != nil {
		utils.WriteError(w, http.StatusInternalServerError, err)
		return
	}
